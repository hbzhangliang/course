...
<c:if test="${excludeOpts==null or !excludeOpts}">
  <c:forEach items="${fn:split(opts, ',')}" var="opt">
    <c:if test="${opt eq 'add'}">
      <button onclick="doInput(null);" type="button" class="ico_add"><fmt:message key="sys.title.add"/></button>
    </c:if>
    <c:if test="${opt eq 'del'}">
      <button onclick="doMultiDel();" type="button" class="ico_del"><fmt:message key="sys.title.delete"/></button>
    </c:if>
  </c:forEach>
</c:if>
