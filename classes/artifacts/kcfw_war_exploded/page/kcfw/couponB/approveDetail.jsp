...
<%--           		<form:input path="meetingCost" cssClass="input_txt w_209"/> --%>
          		<label>(按实际发生费用的发票金额给予一定补贴)</label>
          </td>
        </tr>
        <tr>
       	  <td class="td1"><b>预算额度</b></td>
       	  <fmt:formatNumber value="${bean.bBudgetMoney}" pattern="0" var="b_BudgetMoney"/>
          <td class="td2" colspan='5'><form:input path="bBudgetMoney" value="${b_BudgetMoney}" cssClass="input_txt w_209"/></td>
        </tr> 
         <tr>
      ...
      <table id="corplimitInfo" class="table_list t_hs">
      	<tr>
      		<td width="8%" class="td1"><b>当前年份</b></td>
      		<td width="8%" class="td2"><label></label></td>
      		<td class="td1">单位名称</td>
      		<td class="td2"><label>${bean.corp.unitName}</label></td>
      		<td width="10%" class="td1"><b>总申请金额(元)</b></td>
      		<td width="10%" class="td2"><label></label></td>
      		<td width="10%" class="td1">总金额(元)</td>
      		<td width="10%" class="td2"><label></label></td>
      		<td width="10%" class="td1"><b>余额(元)</b></td>
      		<td width="10%" class="td2"><label></label></td>
      	</tr>
      </table>
...
<!--footer end-->
<script>
   var requiredList = ["#suggest"];
//   var intFields=["#ordernum"];

	function doAction(){
		$.messager.defaults = { ok: "备案", cancel: "不备案",width:'350px',height:'170px'};
		  $.messager.confirm("操作提示", "备案结果？", function (flag) {
	       	  ...
	       	  $.post(base + '/couponMng/couponApprove/approve.htm', {id:couponId,suggest:$("#suggest").val(),flag:flag}, function (data) {
	       		  ...
	       		  setTimeout(function(){
	       			  window.location.href=base+'/couponMng/couponBApprove/list.htm';
	       		  },800)
	       	  });
	      });
  ...
  function init_page(){
	...
	var dt=new Date();
	  ...
	  $("#corplimitInfo tr").eq(0).find("td:eq(5)").text(emptyFormat("${busage.cost}"));
	  ...
	  else{ //不可能走到这步  因为一般企业不可能申请B券
		  btotal="${limit.kj_fullyearB}";
	  }
	  var buse="${busage.cost}";
	  $("#corplimitInfo tr").eq(0).find("td:eq(7)").text((btotal));
	  $("#corplimitInfo tr").eq(0).find("td:eq(9)").text((btotal)-(buse));
