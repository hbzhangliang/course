...
<!--           		<label>(按实际发生费用的发票金额给予一定补贴)</label> -->
          		<fmt:formatNumber value="${bean.meetingCost}" pattern="0" var="meeting_cost"/>
          		<form:label path="meetingCost" cssClass="w_209">${meeting_cost}</form:label>元
          		<label>(按实际发生费用的发票金额给予一定补贴)</label>
          		</td>
...
<script>
  ...
  function init_page(){
	  ...
	  $("#corplimitInfo tr").eq(0).find("td:eq(7)").text(emptyFormat(btotal)-emptyFormat(buse));
