    ...
    <div class="right clearfix">
        ...
        <div class="an">
            <c:if test="${currentUser.type eq 'corp'}">
                <gw:bizSearch fields="项目名称" path="name"  opts="add" />
            </c:if>
            <c:if test="${!(currentUser.type eq 'corp')}">
                <gw:bizSearch fields="项目名称" path="name"  />
            </c:if>
        ...
        <table class="table_01">
            <tr>
                ...
                <th width="6%" class="orderBy">法定代表人</th>
                <th width="6%" class="orderBy">项目负责人</th>
                <th width="6%" class="orderBy">项目联系人</th>
                <th width="8%" class="orderBy">注册时间</th>
                <th width="8%" class="orderBy">注册资本</th>
                <th width="8%" class="orderBy">状态</th>
            ...
            <c:forEach items="${list}" var="bean" varStatus="i">
                <tr id="tr_${bean.id}">
                    ...
                    <td>${bean.fzname}</td>
                    <td>${bean.lxname}</td>
                    <td><fmt:formatDate value="${bean.regDate}" pattern="yyyy年MM月dd日" /></td>
                    <td>${bean.regcaptial}</td>
